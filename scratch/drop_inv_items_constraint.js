// drop_inv_items_constraint.js
// This script uses the backend API to drop the problematic constraint on inv_items
...
async function dropConstraint() {
  try {
    ...
    try {
      ...
      if (!dropRes.ok) {
        throw new Error(`HTTP error! status: ${dropRes.status}`);
      }
    ...
    try {
      ...
      if (!dropRes2.ok) {
        throw new Error(`HTTP error! status: ${dropRes2.status}`);
      }
